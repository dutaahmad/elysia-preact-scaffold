...
- **Icons**: `@phosphor-icons/react` (PascalCase named imports, `size={16|20}`)
...
- **Theme**: `useTheme` hook in `src/hooks/useTheme.ts` + `ThemeToggle` in `src/components/ThemeToggle.tsx`. Sets `document.documentElement.dataset.theme`. New components should respect both modes.
- **SidebarLink**: shared component in `src/app.tsx`, uses `cn()` and `aria-current` for active state. Icon passed as children.
- **CLI templates**: `cli/templates/module.ts` (6 server files), `cli/templates/fe.ts` (client, types, api, list/form pages, app template, home), `cli/templates/project.ts` (scaffold server files). CLI code is NOT typechecked.
...
- **`cn()` rule**: every `class` attribute must use `cn(...)` from `src/lib/utils.ts`, especially when combining multiple classes or conditionals.
- **Icon convention**: import specific icons from `@phosphor-icons/react` (PascalCase, no prefix pattern). `size={20}` for standalone, `size={16}` for inline with text.
