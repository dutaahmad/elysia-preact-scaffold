...
export function PeopleEdit() {
  ...
  const { data: existing } = useQuery({
    ...
    queryFn: () => peopleApi.getById(Number(params!.id)),
  })

  useEffect(() => {
    if (existing) {
      const { id, createdAt, updatedAt, ...rest } = existing as CreatePeople & { id: number; createdAt: string; updatedAt: string }
      setFormData(rest)
    }
  }, [existing])

  const mutation = useMutation({
    mutationFn: (data: CreatePeople) => peopleApi.update(Number(params!.id), data),
    onSuccess: () => {
    ...
    <div class="page">
      <div class="page__header">
        <div class="page__headline">
          <h1 class="page__title">Edit People</h1>
        </div>
